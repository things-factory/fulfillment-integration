...
class FulfillmentCenters extends localize(i18next)(PageView) {
      ...
      actions: [
        {
          ...
          action: this._updateCenters.bind(this)
        },
        {
          title: i18next.t('button.delete'),
          action: this._deleteCenters.bind(this)
        }
      ],
      exportable: {
    ...
    this.config = {
      ...
      columns: [
        ...
        },
        {
          type: 'string',
          name: 'centerId',
          header: i18next.t('field.center-id'),
          record: {
            editable: true
          },
          sortable: false,
  ...
  async _updateCenters() {
    let patches = this.dataGrist.dirtyRecords
    if (patches && patches.length) {
      patches = patches.map(center => {
        let patchField = center.id ? { id: center.id } : {}
        const dirtyFields = center.__dirtyfields__
        for (let key in dirtyFields) {
          patchField[key] = dirtyFields[key].after
        }
        patchField.cuFlag = center.__dirty__
